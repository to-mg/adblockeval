...
class AdblockRules:
    def __init__(
        ...
        if rule_list and cache_file:
            raise ValueError(
                "Cannot use rule_list if cache_file is provided. "
        ...
        if rule_files is not None:
            ...
            if cache_file:
                ...
                try:
                    ...
                    newest_mtime = max(
                        rule_file.stat().st_mtime for rule_file in rule_files
                    )
        ...
        if cache_file and not load_from_cache:
            with cache_file.open("wb") as f:
                pickle.dump(
                    (self.rules, self._index), f, pickle.HIGHEST_PROTOCOL
                )
    ...
    def _parse_rule(self, rule_str):
        ...
        expression = parts[0]
        options = (
            RuleOptions.from_string(parts[1]) if len(parts) == 2 else None
        )
...
class RulesIndex:
    ...
    def _build_index(self):
        ...
        for rule_index, rule in enumerate(self.rules):
            ...
            if (
                not rule_keywords.url_keywords
                and not rule_keywords.domain_keywords
            ):
    ...
    def get_possible_rules(self, url, netloc, domain):
        ...
        if domain:
            domain_opt_rules = set(
                self._domain_opt_index.get_matching_keywords(domain)
            )
        else:
...
class Rule:
    ...
    def match(self, url, netloc, domain, origin=None):
        return bool(
            not self.options or self.options.can_apply_rule(domain, origin)
        )
...
class DomainRule(Rule):
    ...
    def __init__(
        self, expression, options, domain, domain_regexp_obj, path_regexp_obj
    ):
        super().__init__(expression, options)
        self._domain = domain
    ...
    def match(self, url, netloc, domain, origin=None):
        ...
        if match_obj is None:
            return False
        domain_matches = (
            match_obj.start() == 0 or netloc[match_obj.start() - 1] == "."
        )
    ...
    def from_expression(cls, expression, options):
        ...
        domain_regexp_obj = _compile_wildcards(
            domain, prefix=r"", suffix="$", lazy=True
        )
        if path:
            prefix = "^" if path.startswith("/") else ""
            path_regexp_obj = _compile_wildcards(
                path, prefix=prefix, suffix="", lazy=True
            )
        else:
            path_regexp_obj = None
        return cls(
            expression, options, domain, domain_regexp_obj, path_regexp_obj
        )
...
class SubstringRule(Rule):
    ...
    def match(self, url, netloc, domain, origin=None):
        ...
        if not self._regexp_obj.pattern:
            return False
...
class RuleOptions:
    ...
    def can_apply_rule(self, domain, origin):
        if self.exclude_domains and _contains_subdomain(
            domain, self.exclude_domains
        ):
            return False
        if self.include_domains and not _contains_subdomain(
            domain, self.include_domains
        ):
            return False
        return True
    ...
    def __str__(self):
        ...
        if self.exclude_domains or self.include_domains:
            ...
            if self.exclude_domains:
                domain_list += sorted(
                    f"~{domain}" for domain in self.exclude_domains
                )
    ...
    def from_string(cls, option_str):
        ...
        for option_part in option_parts:
            ...
            if option_part in cls.AVAILABLE_OPTIONS:
                ...
                option_part[0] == "~"
                and option_part[1:] in cls.AVAILABLE_OPTIONS
            ):
                ...
                for domain in domains:
                    ...
                    domain_list.append(
                        domain if domain[0] != "~" else domain[1:]
                    )
        return cls(
...
def _compile_wildcards(
    expression, prefix="", suffix="", match_case=False, lazy=False
):
...
def _is_subdomain(subdomain, domain):
    return subdomain == domain or (
        ...
        and len(subdomain) > len(domain)
        and subdomain[-len(domain) - 1] == "."
    )
